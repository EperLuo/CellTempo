...
from .transformer import Transformer1DModel
    ...
    def __init__(
            ...
            if not dual_cross_attention:
                attentions.append(
                    Transformer1DModel(
                        ...
                        out_features // num_attention_heads,
                        in_channels=out_features,
                        # num_vector_embeds=32,
                        # sample_size=out_channels,
                        num_layers=transformer_layers_per_block[i],
                        cross_attention_dim=cross_attention_dim,
                        norm_num_groups=resnet_groups,
                        use_linear_projection=use_linear_projection,
        ...
        if add_downsample:
            self.downsamplers = nn.ModuleList(
                [
                    Downsample1D(
                        out_features, use_conv=True, out_channels=out_features, padding=downsample_padding, name="op"
                    )
                ]
            )
        else:
...
class CrossAttnUpBlock1D(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            res_skip_channels = in_channels if (i == num_layers - 1) else out_channels
            resnet_in_channels = prev_output_channel if i == 0 else out_channels
            in_feature = in_features if (i != 0) else out_features
            out_feature = out_features if i != num_layers - 1 else in_features
            ...
            if not dual_cross_attention:
                attentions.append(
                    Transformer1DModel(
                        num_attention_heads,
                        in_feature // num_attention_heads,
        ...
        if add_upsample:
            self.upsamplers = nn.ModuleList([Upsample1D(in_features, use_conv=True, out_channels=in_features, interpolate=False)])
        else:
            self.upsamplers = None
