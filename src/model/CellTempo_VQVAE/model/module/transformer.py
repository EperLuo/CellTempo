...
class Transformer1DModel(LegacyModelMixin, LegacyConfigMixin):
    """
    A 1D Transformer model for RNAseq-like data.
    ...
    def _init_continuous_input(self, norm_type):
        ...
        self.norm = torch.nn.LayerNorm(
            self.in_channels, eps=1e-6, elementwise_affine=True
        )
    ...
    def _operate_on_continuous_inputs(self, hidden_states):
        # batch, _, feature_dim = hidden_states.shape
        hidden_states = self.norm(hidden_states)

        if not self.use_linear_projection:
            hidden_states = self.proj_in(hidden_states)
            inner_dim = hidden_states.shape[2]
            # hidden_states = hidden_states.permute(0, 2, 1).reshape(batch, feature_dim, inner_dim)
        else:
            inner_dim = hidden_states.shape[2]
            # hidden_states = hidden_states.permute(0, 2, 1).reshape(batch, feature_dim, inner_dim)
            hidden_states = self.proj_in(hidden_states)
    ...
    def _get_output_for_continuous_inputs(self, hidden_states, residual, batch_size, inner_dim):
        if not self.use_linear_projection:
            # hidden_states = (
