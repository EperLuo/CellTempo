...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="rna",
        help="Train on RNA-seq (rna) or ATAC-seq (atac).",
    )
    parser.add_argument(
        "--vq",
        default=False,
        action="store_true",
        help="Use vq-vae or standard vae",
    )
    parser.add_argument(
        "--cache_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        default="celltempo",
        help=(
            "The `project_name` argument passed to Accelerator.init_trackers. Wandb project name."
        ),
    )
    parser.add_argument(
        "--run_name",
...
def main():
    ...
    # num_vq_embeddings: the size of the code book; latent_dim: the dim of encoder output
    if args.pretrained_model_name_or_path is not None:   # if finetune vq-model
        model = VQModel.from_pretrained(args.pretrained_model_name_or_path, cvq_distance = 'cos',cvq_anchor='probrandom')
    else:
        model = VQModel(num_vq_embeddings=512,vq_embed_dim=24,latent_dim = 624, 
                        hidden_dim=[1536,1536,1024], num_genes=args.num_gene, cvq_distance = 'cos',
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                if args.use_ema:
                    ema_model.save_pretrained(os.path.join(output_dir, "vqmodel_ema"), safe_serialization=False)
                vqmodel = models[0]
                vqmodel.save_pretrained(os.path.join(output_dir, "vqmodel"), safe_serialization=False)
                weights.pop()
    ...
    def collate_fn(examples):
        ...
        if 'gene_id' in examples[0].keys():
            gene_id = torch.stack([example["gene_id"] for example in examples if example["gene_id"] is not None])
            gene_id = gene_id.to(memory_format=torch.contiguous_format).float()
        else:
            gene_id = None
    ...
    if resume_from_checkpoint:
        if resume_from_checkpoint != "latest":
            ...
            path = os.path.join(args.output_dir, path) if path is not None else path
        ...
        if path is None:
            ...
            global_step = int(os.path.basename(path).split("-")[1]) #if path.split('/')[-2] == args.output_dir.split('/')[-1] else 0
    ...
    # mask_benchmark = torch.tensor(np.load('/stor/lep/workspace/scdiffusion_v2/scripts/mask_benchmark.npz')['data'],device=model.device)
    def norm_total(array, target_sum = 1e4):        
        current_sum = torch.sum(array,dim=1)[...,None] if len(array.shape)>1 else torch.sum(array)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            gene_id = batch["gene_id"] if batch["gene_id"] is not None else None
            data_time_m.update(time.time() - end)

            fmap, commit_loss = model(pixel_values, return_dict=False, mode=args.data_type, vq=args.vq, gene_id=gene_id)

            # if generator_step:
            with accelerator.accumulate(model):
                # reconstruction loss. Pixel level differences between input vs output
                if args.vae_loss == "l2":
                    fmap = norm_total(F.relu(fmap),1e4)
                    ...
                    fmap = F.softmax(fmap, dim=1) * pixel_values.sum(1).unsqueeze(-1) #2000
                    px = normal(fmap, F.softplus(model.module.theta).sqrt())
                    loss = - px.log_prob(pixel_values).sum(1).mean() #* 0.0001 # 0.0002 0.0002
                else:
                    raise ValueError('invalid vae loss type')
                re_loss = loss
    ...
    if accelerator.is_main_process:
        ...
        model.save_pretrained(os.path.join(args.output_dir, "vqmodel"), safe_serialization=False)
